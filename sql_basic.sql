...
SELECT
  region,
  life_expectancy as 平 均 寿 命,
  AVG(population) AS 平 均 人 口
FROM
  countries
GROUP BY
  region;
...
SELECT
  region,
  MAX(life_expectancy) as 平 均 寿 命,
  MAX(population) AS 平 均 人 口
FROM
  countries
GROUP BY
  region;
...
SELECT
  MIN(surface_area) AS ア ジ ア の 最 小 面 積
FROM
  countries
WHERE
  continent LIKE '%asia%' -- 問24
  -- アジア大陸の表面積の合計を表示してください。
SELECT
  SUM(surface_area) AS ア ジ ア 大 陸 の 表 面 積 の 合 計
FROM
  countries
WHERE
  continent LIKE '%asia%';
...
FROM
  celebrities
  LEFT JOIN countries ON celebrities.name = countries.name -- 問28
  -- 全ての有名人の名前,国名、第一言語を出力してください。
SELECT
  celebrities.name,
  celebrities.country_code as name,
  language
FROM
  celebrities
...
SELECT
  celebrities.country_code AS 国 名,
  AVG(celebrities.age) AS 平 均 寿 命
FROM
  countries
